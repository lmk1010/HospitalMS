...
function convertServerMenuToRouteRecordStringComponent(
  ...
  const normalizedMenuList = Array.isArray(menuList)
    ? menuList
    : Array.isArray((menuList as any)?.data)
      ? (menuList as any).data
      : Array.isArray((menuList as any)?.data?.data)
        ? (menuList as any).data.data
        : [];

  normalizedMenuList.forEach((menu) => {
    // 处理外链菜单（顶级或子级）
    ...
    const rawComponent = typeof menu.component === 'string' ? menu.component : '';
    let normalizedComponent = rawComponent;
    if (!normalizedComponent && !menu.children?.length) {
      console.warn(`menu component is empty, fallback to not-found: ${menu.name}, id: ${menu.id}`, menu);
      normalizedComponent = '/_core/fallback/not-found';
    }

    // add by 芋艿：处理 menu.component 中的 query 参数
    // https://doc.vben.pro/guide/essentials/route.html#query
    let query: Record<string, string> | undefined;
    const queryIndex = normalizedComponent.indexOf('?');
    if (queryIndex !== -1) {
      // 提取 query 字符串并解析为对象
      const queryString = normalizedComponent.slice(queryIndex + 1);
      query = Object.fromEntries(new URLSearchParams(queryString).entries());
      // 移除 component 中的 query 部分
      normalizedComponent = normalizedComponent.slice(0, queryIndex);
    }
