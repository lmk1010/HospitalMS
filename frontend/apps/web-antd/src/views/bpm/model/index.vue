...
async function getList() {
  ...
  try {
    ...
    const orderedCategoryList = [...categoryList].toSorted(
      (left: any, right: any) => {
        if (left.code === 'HOSPITAL_WORKFLOW') return -1;
        if (right.code === 'HOSPITAL_WORKFLOW') return 1;
        return (left.sort || 0) - (right.sort || 0);
      },
    );
    // 按照 category 聚合
    categoryGroup.value = orderedCategoryList.map((category: any) => ({
      ...category,
