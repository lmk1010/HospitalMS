...
const CONTOUR_WORKFLOW_KEY = 'contour-review' as const;
...
const reviewList = ref<HospitalContourReviewApi.ContourReview[]>([]);
const doctorOptions = ref<HospitalDoctorApi.Doctor[]>([]);
const selectedReview = ref<HospitalContourReviewApi.ContourReview>();
...
const canApprove = computed(() =>
  hasAccessByCodes(['hospital:contour-review:approve']),
);
const canReject = computed(() =>
  hasAccessByCodes(['hospital:contour-review:reject']),
);
...
const readonlyMessage = computed(() => {
  ...
  if (status === 'APPROVED') return '当前审核已通过，右侧信息只读。';
  if (status === 'REJECTED') return '当前审核已退回，右侧信息只读。';
  return '';
});
...
const reviewWorkflowStatusList = computed(() => ['REVIEWING']);
...
function getReviewStatusCode(review?: HospitalContourReviewApi.ContourReview) {
...
function getWorkflowCurrentTitle(status?: string) {
  ...
  if (status === 'REJECTED') return '靶区勾画';
  return '勾画审核';
}
...
function fillAuditForm(review?: HospitalContourReviewApi.ContourReview) {
  Object.assign(auditForm, {
    id: review?.id,
...
async function loadReviewList(preserveSelection = true) {
  ...
  try {
    ...
    const result = await getContourReviewPage({
      id: focusId,
      ...
      workflowStatusList: [...reviewWorkflowStatusList.value],
    } as any);
...
async function loadReviewDetail(id: number) {
  ...
  approvalProcessId.value = '';
  try {
    const review = await getContourReview(id);
    selectedReviewId.value = id;
    selectedReview.value = review;
    fillAuditForm(review);

    const [taskResult, patientResult] = await Promise.allSettled([
      review.contourTaskId
        ? getContourTask(review.contourTaskId)
        : Promise.resolve(undefined),
      review.patientId
        ? getPatient(review.patientId)
        : Promise.resolve(undefined),
    ]);

    selectedTask.value =
      taskResult.status === 'fulfilled' ? taskResult.value : undefined;

    if (
      !review.patientId &&
      selectedTask.value?.patientId &&
      patientResult.status !== 'fulfilled'
    ) {
      try {
        patientDetail.value = await getPatient(selectedTask.value.patientId);
      } catch {
        patientDetail.value = undefined;
      }
    } else {
    ...
    if (
      selectedTask.value?.ctAppointmentId &&
      selectedTask.value.ctAppointmentId > 0
    ) {
      try {
        appointmentDetail.value = await getCtAppointment(
          selectedTask.value.ctAppointmentId,
        );
      } catch {
        appointmentDetail.value = undefined;
...
function buildAuditPayload():
  | HospitalContourReviewApi.ContourReviewAudit
  | undefined {
...
async function handleApprove() {
  ...
  if (!payload) {
    message.warning('请先选择审核医生');
    return;
  }
  const hideLoading = message.loading({
    content: `正在提交审核通过【${selectedReview.value?.bizNo || ''}】`,
    duration: 0,
  });
  actionLoading.value = true;
  try {
    await approveContourReview(payload);
...
async function handleReject() {
  ...
  const hideLoading = message.loading({
    content: `正在提交审核退回【${selectedReview.value?.bizNo || ''}】`,
    duration: 0,
  });
  actionLoading.value = true;
  try {
    await rejectContourReview(payload);
    ...
    <div class="contour-board">
      <div class="contour-toolbar">
        <div class="contour-toolbar__heading">
          <div class="contour-toolbar__title">勾画审核</div>
          <div class="contour-toolbar__subtitle">
            选中左侧审核单后，在右侧完成审核并提交结果。
          </div>
        </div>
      ...
      <div class="contour-main">
        <section class="contour-panel contour-list-panel">
          <div class="contour-panel__header">
            <div class="contour-panel__title">勾画审核</div>
          ...
          <a-spin :spinning="listLoading">
            <div class="contour-table-wrap">
              <table v-if="reviewList.length > 0" class="contour-table">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td>#{{ displayText(item.contourTaskId) }}</td>
              ...
              <a-empty v-else description="暂无勾画审核" />
            </div>
          </a-spin>
        ...
        <section class="contour-panel contour-detail-panel">
          <div
            :class="[
              'contour-patient-bar',
              { 'contour-patient-bar--empty': !selectedReview },
            ]"
          >
            ...
            <div class="contour-patient-bar__main">
              ...
              <div class="contour-patient-bar__meta">
                ...
                >
                <span>审核单号：{{ displayText(selectedReview?.bizNo) }}</span>
                <span>任务单号：{{ displayText(selectedTask?.bizNo) }}</span>
                <span
                  >勾画医生：{{
                    displayText(selectedTask?.contourDoctorName)
                  }}</span
                >
              </div>
            </div>
            ...
            <div class="contour-workspace">
              <aside class="contour-timeline">
                ...
                <div
                  ...
                  :key="item.key"
                  :class="['contour-timeline__item', `is-${item.tone}`]"
                >
                  <div
                ...
                <div class="contour-sheet__paper">
                  <div class="contour-sheet__header">
                    ...
                    <div class="contour-sheet__title-box">
                      <div class="contour-sheet__title">勾画审核</div>
                  ...
                  <div class="contour-section">
                    ...
                    <div
                      :class="[
                        'contour-remote',
                        { 'is-empty': !selectedTask?.attachmentUrl },
                      ]"
                    >
                      <template v-if="selectedTask?.attachmentUrl">
                        <div class="contour-remote__text">
                          {{ selectedTask.attachmentUrl }}
                        </div>
                        <a :href="selectedTask.attachmentUrl" target="_blank"
                          >打开附件</a
                        >
                      </template>
                  ...
                  <div class="contour-section">
                    ...
                    <div
                      class="contour-sheet__grid contour-sheet__grid--2 contour-sheet__grid--slab"
                    >
                      <div class="contour-field">
                        <label>任务单号</label
                        ><span>{{ displayText(selectedTask?.bizNo) }}</span>
                      </div>
                      <div class="contour-field">
                        <label>版本号</label
                        ><span>{{ displayText(selectedTask?.versionNo) }}</span>
                      </div>
                      <div class="contour-field">
                        <label>勾画医生</label
                        ><span>{{
                      ...
                      <div class="contour-field contour-field--full">
                        <label>任务备注</label
                        ><span>{{ displayText(selectedTask?.remark) }}</span>
                      </div>
                    </div>
                  </div>

                  <div class="contour-section">
                    <div class="contour-section__title">审核信息</div>
                    <a-alert
