...
const activeStatus = ref<ReviewStatusKey>('DRAFT');
...
const filters = reactive({
  bizNo: '',
  patientName: '',
  verifyUserId: undefined as number | undefined,
});
...
const canEditCurrent = computed(() => {
  return getReviewStatusCode(selectedReview.value) === 'DRAFT';
});

const readonlyMessage = computed(() => {
  const status = getReviewStatusCode(selectedReview.value);
  if (status === 'APPROVED') return '当前验证已通过，右侧信息只读。';
  if (status === 'REJECTED') return '当前验证未通过，右侧信息只读。';
  return '';
});

const statusTabs = computed(() => {
  const buildItem = (key: ReviewStatusKey, label: string) => ({ key, label });
  return [buildItem('DRAFT', '待验证')];
});
...
const nodeFormValues = computed<Record<string, any>>(() => ({
  handleOpinion:
    auditForm.reportUrl || selectedReview.value?.reportUrl || selectedReview.value?.remark,
  handleResult:
    selectedReview.value?.verifyResult || selectedReview.value?.workflowStatus,
  handlerName: selectedReview.value?.verifyUserName,
  ...
  reportUrl: auditForm.reportUrl || selectedReview.value?.reportUrl,
  totalDose:
    selectedPlanDesign.value?.totalDose || selectedPlanApply.value?.prescriptionDose,
  verifyDeviceName:
    auditForm.verifyDeviceName || selectedReview.value?.verifyDeviceName,
}));
...
function getReviewStatusCode(review?: ReviewRecord) {
  return review?.verifyResult || review?.workflowStatus || '';
}
...
function fillAuditForm(review?: ReviewRecord) {
  Object.assign(auditForm, {
    ...
    verifyDeviceName: review?.verifyDeviceName || '',
    verifyUserId: review?.verifyUserId,
  });
}
...
async function loadReviewList(preserveSelection = true) {
  ...
  try {
    ...
    const result = await props.api.getPage({
      ...
      verifyUserId: filters.verifyUserId,
...
function buildAuditPayload(): ReviewAuditPayload | undefined {
  if (!selectedReview.value?.id || !auditForm.verifyUserId) {
    return undefined;
  }
  return {
    id: selectedReview.value.id,
    remark: auditForm.remark.trim(),
    reportUrl: auditForm.reportUrl.trim(),
    verifyDeviceName: auditForm.verifyDeviceName.trim(),
    verifyUserId: auditForm.verifyUserId,
  };
}
...
async function handleReject() {
  ...
  const hideLoading = message.loading({
    content: `正在标记不通过【${selectedReview.value?.bizNo || ''}】`,
    duration: 0,
  });
  actionLoading.value = true;
  try {
...
function handleReset() {
  ...
  filters.bizNo = '';
  filters.patientName = '';
  filters.verifyUserId = undefined;
  pager.pageNo = 1;
  void loadReviewList(false);
}
    ...
    <div class="contour-board">
      <div class="contour-toolbar">
        ...
        <div class="contour-toolbar__filters">
          ...
          />
          <a-select
            v-model:value="filters.verifyUserId"
            allow-clear
            class="w-[140px]"
              ...
              <table v-if="reviewList.length > 0" class="contour-table">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td>{{ displayText(item.verifyUserName) }}</td>
                    <td>
                    ...
                    <td>
                      <div class="contour-table__date">
                        {{ formatDate(item.verifyTime || item.createTime) }}
                      </div>
              ...
              <div class="contour-patient-bar__meta">
                ...
                <span>
                  {{ props.config.actorLabel }}：{{
                    displayText(selectedReview?.verifyUserName)
                  }}
                </span>
              </div>
            </div>
                  ...
                  <div class="contour-section">
                    ...
                    >
                      <div class="contour-editor-item">
                        ...
                        <a-select
                          v-model:value="auditForm.verifyUserId"
                      ...
                      <div class="contour-editor-item">
                        <label>{{
                          props.config.deviceLabel || '验证设备'
                        }}</label>
                        <a-input
                          v-model:value="auditForm.verifyDeviceName"
                          :disabled="!selectedReview || !canEditCurrent"
                          :placeholder="
                            props.config.devicePlaceholder ||
                            '请输入验证设备名称'
                          "
                          size="small"
                        />
                      ...
                      >
                        ...
                        <a-input
                          v-model:value="auditForm.reportUrl"
                          :disabled="!selectedReview || !canEditCurrent"
