...
public class BpmSimpleModelNodeVO {
    ...
    @Schema(description = "医院节点扩展配置", example = "{}")
    private HospitalSetting hospitalSetting;

    @Schema(description = "医院节点扩展配置")
    @Data
    public static class HospitalSetting {

        @Schema(description = "业务分类", example = "计划管理")
        private String bizCategory;

        @Schema(description = "节点页面编码", example = "plan-verify")
        private String pageCode;

        @Schema(description = "节点页面路径", example = "/hospital-flow/hospital-plan/verify")
        private String pagePath;

        @Schema(description = "绑定医院表单编号", example = "1")
        private Long formId;

        @Schema(description = "流转记录页面编码", example = "plan-verify")
        private String flowRecordPageCode;

        @Schema(description = "功能系统编码", example = "计划管理")
        private String functionSystemCode;

        @Schema(description = "前置节点编号", example = "Activity_1")
        private String previousNodeId;

        @Schema(description = "子流程标识", example = "hospital_radiotherapy_external_beam_gz")
        private String childProcessKey;

        @Schema(description = "是否自动跳过", example = "false")
        private Boolean autoSkip;

        @Schema(description = "是否结束节点", example = "false")
        private Boolean endNode;

        @Schema(description = "是否打印提交人签名", example = "true")
        private Boolean printSubmitterSignature;

        @Schema(description = "可编辑角色编号数组", example = "[1,2]")
        private List<Long> editRoleIds;

        @Schema(description = "操作按钮角色编号数组", example = "[1,2]")
        private List<Long> buttonRoleIds;

        @Schema(description = "关联文书表单编号数组", example = "[1,2]")
        private List<Long> documentFormIds;

        @Schema(description = "启用的动作按钮编号数组", example = "[1,2]")
        private List<Integer> actionButtonIds;

        @Schema(description = "启用二次确认的动作按钮编号数组", example = "[1,2]")
        private List<Integer> secondaryConfirmButtonIds;

        @Schema(description = "超时规则列表", example = "[]")
        private List<HospitalTimeoutRule> timeoutRules;

    }

    @Schema(description = "医院节点超时规则")
    @Data
    public static class HospitalTimeoutRule {

        @Schema(description = "规则编号", example = "rule_1")
        private String id;

        @Schema(description = "超时动作", example = "1")
        private Integer action;

        @Schema(description = "超时数值", example = "6")
        private Integer duration;

        @Schema(description = "时间单位", example = "HOUR")
        private String unit;

        @Schema(description = "最多提醒次数", example = "1")
